...
- 안내 시간: 각 동작은 근거 기반 권장 시간(정적 스트레칭 20~30초, 친 턱은 짧게)만큼 표시된다. 기본 5동작 총 ~2분. 배열 항목에 `d`(초)가 없으면 설정값(기본 12초, Settings에서 3–120초)을 폴백으로 사용
- 동작 목록: 스크립트 상단 `STRETCHES` 배열(`t`동작·`s`설명·`d`초)에서 편집 — 근거: ACSM 2011·Mayo Clinic·물리치료 가이드 (메뉴 Settings → "스크립트 편집기 열기")
